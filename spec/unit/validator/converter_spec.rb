...
describe Validator::Converter do
  ...
  let(:tmpdir) { 'fake-tmpdir' }
  let(:config_dir) { File.join(tmpdir, 'jobs', 'openstack_cpi', 'config') }
  subject { Validator::Converter.new(File.join(config_dir, 'cacert.pem')) }
  ...
  describe 'end to end keystone v3 config' do
    ...
    it 'produces the expected result for the given input' do
      ...
      expect(subject.to_cpi_json(config.openstack)).to eq(expected_cpi_config)
    end
  end
  ...
  describe '.to_cpi_json' do
    ...
    describe 'conversions' do
      context 'when keystone v3 is being used' do
        it "emits 'project' and 'domain' but not 'tenant'" do
          ...
          rendered_cpi_config = subject.convert_and_apply_defaults(v3_config_with_tenant)
        ...
        context "when 'auth_url' does not end with '/auth/tokens'" do
          it "appends 'auth/tokens' to 'auth_url' parameter" do
            rendered_cpi_config = subject.convert_and_apply_defaults(complete_config)
      ...
      context 'when keystone v2 is being used' do
        ...
        it "emits 'tenant' and not 'domain' or 'project'" do
          ...
          rendered_cpi_config = subject.convert_and_apply_defaults(v2_config_with_domain_and_project)
      ...
      describe 'default values' do
        before do
          allow(subject).to receive(:openstack_defaults).and_return({'default_key' => 'default_value'})
        end
        context 'when value is not set and default value exists' do
          it 'uses default value' do
            expect(complete_config).to_not include(subject.openstack_defaults)
            expect(subject.convert_and_apply_defaults(complete_config)).to include(subject.openstack_defaults)
          end
        end
        ...
        context 'when value is manually set for a key which has default value available' do
          ...
          it 'uses the manually set value' do
            expect(subject.convert_and_apply_defaults(complete_config_including_overridden_defaults)).to include('default_key' => 'my-value')
          end
        end
      end
      ...
      context 'when connection_options' do
        ...
        context '.ca_cert is given' do
          ...
          it "replaces 'ca_cert' with 'ssl_ca_file'" do
            rendered_cpi_config = subject.convert_and_apply_defaults(config_with_ca_cert)
        ...
        [{ name: 'nil', value: nil}, { name: 'empty', value: ''}].each do |falsy_value|
          ...
          context ".ca_cert is given #{falsy_value[:name]}" do
            ...
            it "removes 'ca_cert'" do
              rendered_cpi_config = subject.convert_and_apply_defaults(config_with_nil_ca_cert)
    ...
    describe 'registry configuration' do
      it "uses the next free ephemeral port" do
        ...
        rendered_cpi_config = subject.to_cpi_json(complete_config)
